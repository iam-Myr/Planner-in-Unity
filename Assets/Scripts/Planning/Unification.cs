...
namespace Planning
{
    public static class Unification
    {
        ...
        private static bool Unify_Pointer(Pointer point, object x)
        {
            ...
            // Check bans BEFORE binding or setting
            if (point.IsBanned(x))
                Debug.Log($"<color=RED>Unification failed: {point.ToString()} is banned from being {x.ToString()}</color>");
                return false;

            // if point not bound, bind to x (either to pointer or to value)
            if (x is Pointer p) // unbound pointer
                point.BindTo(p);
            else              // concrete value
                point.Set(x);
